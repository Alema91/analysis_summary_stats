...
do
	...
	echo -e "$(echo ${in} | sed 's/_/-/g')\t$(echo 'NA')\t$(grep 'total_reads' */fastp/${in}.fastp.json | head -n1 | tail -n1 | cut -d ':' -f2 | sed 's/,//g')\t$(grep 'total_reads' */fastp/${in}.fastp.json | head -n2 | tail -n1 | cut -d ':' -f2 | sed 's/,//g')\t$(echo 'NA')\t$(echo 'NA')\t$(echo 'NA')\t$(echo 'NA')\t$(echo 'NA')\t$(echo 'NA')\t$(echo 'NA')\t$(echo 'NA')\t$(echo 'NA')\t$(cat df_nsper100.csv | grep ${in} | cut -f2)\t$(echo 'NA')\t$(echo 'NA')\t$(echo 'NA')\t$(echo 'NA')" >> summary_mqc_mod.csv
done
